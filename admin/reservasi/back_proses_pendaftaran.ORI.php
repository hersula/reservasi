...
function format_id_transaction($name_outlet, $outletID)
{
    ...
    if ($query_cek_this_day->num_rows >= 1) {
        ...
        //                       WHERE Month(createdAt) = '$thisMonth' AND Year(createdAt) ='$thisYear'");
         $query = $conn->query("SELECT transaksiID as noAntrian FROM transaksi_rawat_jalan WHERE DAY(createdAt) = '$thisDay' AND Month(createdAt) = '$thisMonth' AND Year(createdAt) ='$thisYear' ORDER BY id DESC  LIMIT 1");
...
//if ($idTransaction == '' || $name == '' || $address == '' || $dateOfBirth == '' || $placeOfBirth == '' || $phone == '' || $gejala == '') {
if ($idTransaction == '' || $name == '' || $address == '' || $dateOfBirth == '' || $gejala == '') {
    # code...
    ...
    if($select_id_transaction->num_rows == 1) {
        ...
        if ($input_transaction) {
            ...
            $sql_query1 = "insert into log_file (karyawanID, waktu, infoActivity) 
                            values ('$_SESSION[idKaryawan]',NOW(),'Admin melakukan pendaftaran pasien dan transaksi riwayat tindakan') ";
