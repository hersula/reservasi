...
if(isset($_GET["id"])){
    ...
    if(mysqli_num_rows($cekHasilRKL) > 0) {
        $sql = "select k.id,k.outletID, k.name, k.email, k.phone, 
                k.password, krl.rolesID as 'rolesID', 
                r.roleName from master_karyawan k 
                join karyawan_role_list krl
...
if(isset($_POST["simpan"])){
  ...
  if($error == 0){
      $sql2 = "update master_karyawan set outletID='$outlet', name='$name', 
                email='$email', phone='$phone' where id='$idKaryawan'";
      mysqli_query($conn, $sql2);

      $sql3 = "delete from karyawan_role_list
               where karyawanID='$idKaryawan'";
      mysqli_query($conn, $sql3);

      foreach ($_POST['role'] as $value) {
          $sql4= "insert into karyawan_role_list (karyawanID, rolesID, status) 
                  values ('$idKaryawan', '".$value."', 'Aktif')";
          mysqli_query($conn, $sql4);
      }
      
      $sql_query1 = "insert into log_file (karyawanID, waktu, infoActivity) 
                    values ('$_SESSION[idKaryawan]',NOW(),'Ubah data karyawan') ";
                  ...
                  <div class="col-md-6">
                    <div class="form-group ">
                      ...
                      -->
                        ...
                        <input readonly required type="password" class="form-control" 
