            ...
            <div class="card">
                <div class="card-body">
                        ...
                        <div class="row justify-content-between">
                            <div class="col-xs-4">
                                <div class="input-group mb-3">
                                    <input type="date" class="form-control form-control-sm" placeholder="Input outlet name" aria-label="Recipient's username" aria-describedby="button-addon2" name="date_filter" value="<?= $_POST['date_filter'] ?>">
                    ...
                    <div class="table-responsive">
                        <table id="dataReportDashboard" class="table table-striped text-xs">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                ...
                                if($date_filter == '') {
                                    $query = $conn->query("SELECT transaksi_rawat_jalan.id, transaksi_rawat_jalan.transaksiID, master_pasien.nik, master_pasien.name name_pasien, master_pasien.passport, master_tindakan.name name_tindakan, master_outlet.name name_outlet, master_payment.namePayment, transaksi_rawat_jalan.tglTindakan, transaksi_rawat_jalan.grandTotal FROM transaksi_rawat_jalan JOIN master_pasien ON transaksi_rawat_jalan.pasienID = master_pasien.id JOIN master_tindakan ON transaksi_rawat_jalan.tindakanID = master_tindakan.id
                                ...
                                AND transaksi_rawat_jalan.tglTindakan = '$date' order by id desc") or die($conn->error);
                                    ...
                                    $query = $conn->query("SELECT transaksi_rawat_jalan.id, transaksi_rawat_jalan.transaksiID, master_pasien.nik, master_pasien.name name_pasien, master_pasien.passport, master_pasien.name name_pasien, master_tindakan.name name_tindakan, master_outlet.name name_outlet, master_payment.namePayment, transaksi_rawat_jalan.tglTindakan, transaksi_rawat_jalan.grandTotal FROM transaksi_rawat_jalan JOIN master_pasien ON transaksi_rawat_jalan.pasienID = master_pasien.id JOIN master_tindakan ON transaksi_rawat_jalan.tindakanID = master_tindakan.id
                                JOIN master_outlet ON transaksi_rawat_jalan.outletID = master_outlet.id JOIN master_payment ON transaksi_rawat_jalan.paymentType = master_payment.id WHERE transaksi_rawat_jalan.status='$status' AND transaksi_rawat_jalan.outletID='$outletID' 
                                ...
                                while ($result = $query->fetch_array()) { ?>
                                    ...
                                    <tr>
                                        <td class="text-right"><?= $result['transaksiID'] ?></td>
                                        <td class="text-right"><?php if($result['nik'] == '') {echo $result['passport'];} else {echo $result['nik'];} ?></td>
                                        <td><?= $result['name_pasien'] ?></td>
                                        <td><?= $result['name_tindakan'] ?></td>
                                        <td><?= $result['name_outlet'] ?></td>
                                        <td class="text-right"><?= $result['tglTindakan'] ?></td>
                                        <td><?= $result['namePayment'] ?></td>
                                        <td class="text-right"><?= number_format($result['grandTotal']) ?></td>
                                    </tr>

                                <?php   } ?>
                                <!--<h2><small>Total : </small>Rp <?= number_format(get_sum_price($conn, $status, $outletID, $date_filter, $date)); ?></h2>-->
                            </tbody>
                            <tfooter>
                                <td colspan="6"><center><label style="visibility: hidden;">&nbsp;</label></center></td>
                                <td class="text-left" style="font-weight:bold;"><center>TOTAL</center></td>
                                <td class="text-right"><h6><?= number_format(get_sum_price($conn, $status, $outletID, $date_filter, $date)); ?></h6></td>
                            </tfooter>
                        </table>
                    </div>
                </div>
                                    ...
                                    <?php 
                                        ...
                                        while($result = $query_pembayaran->fetch_array()) { ?>
                                            <tr>
                                                ...
                                                <td class="text-right">Rp <?= number_format($result['grandTotal']) ?></td>
                                            </tr>
                                       <?php }
