...
<head>
    ...
    <style>
       ...
       /*dibawah ini adalah css untuk barcode batang*/
        /*.element-label1 {*/
            /*border: 1px solid black;*/
        /*    padding: 1mm;*/
        /*    text-align: center;*/
...
<body>
    ...
    $barcodeLab = isset($_GET['barcodeLab']) ? $_GET['barcodeLab'] : '';
    
    $query      = $conn->query("SELECT master_pasien.name, master_pasien.dateOfBirth 
    FROM `transaksi_rawat_jalan` JOIN master_pasien ON transaksi_rawat_jalan.pasienID = master_pasien.id WHERE `barcodeLab` = '$barcodeLab'") or die ($conn->error);
    ...
    if ($barcodeLab != '') {
        ...
        $format_barcode = $barcodeLab;
        $format_barcode_new = $barcodeLab . '/' . $namesubs;
