...
if(!in_array("Superadmin", $arrayRoleName) && !in_array("Laboratorium", $arrayRoleName)) {
  ...
  /*log file*/
    $sql_query1 = "insert into log_file (karyawanID, waktu, infoActivity) 
        values ('$_SESSION[idKaryawan]',NOW(),'Lihat halaman hasil PCR') ";
        ...
        <div class="content-wrapper">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="dataHasilTesAntigen" class="table table-bordered table-striped text-xs">
                            <thead>
                                <tr>
                                    <th>Kode Barcode</th>
                                    <th>NIK</th>
                            ...
                            <tbody>
                               <?php
                                ...
                                if($date_filter != '' && $date_filter2 != '') {
                                    ...
                                    master_tindakan.name name_tindakan, transaksi_rawat_jalan.createdAt waktu_tindakan, transaksi_rawat_jalan.sampleTime FROM `transaksi_rawat_jalan` 
                                    join master_pasien on transaksi_rawat_jalan.pasienID = master_pasien.id join master_outlet 
                                    on master_outlet.id = transaksi_rawat_jalan.outletID join master_tindakan on transaksi_rawat_jalan.tindakanID = master_tindakan.id WHERE not transaksi_rawat_jalan.status = 'Failed' and not transaksi_rawat_jalan.status = 'Paid' and master_tindakan.typeTindakan = 'PCR' and not transaksi_rawat_jalan.barcodeLab = 'Belum dibarcode' and transaksi_rawat_jalan.tglTindakan like '%$date_now%' order by transaksi_rawat_jalan.transaksiID desc";
                                    } else {
                                        $query = "SELECT transaksi_rawat_jalan.transaksiID, transaksi_rawat_jalan.status status_tindakan, transaksi_rawat_jalan.pasienID, transaksi_rawat_jalan.barcodeLab, master_pasien.nik, master_pasien.phone, master_pasien.name name_pasien, master_outlet.name,
                                    master_tindakan.name name_tindakan, transaksi_rawat_jalan.createdAt waktu_tindakan, transaksi_rawat_jalan.sampleTime FROM `transaksi_rawat_jalan` 
                                    join master_pasien on transaksi_rawat_jalan.pasienID = master_pasien.id join master_outlet 
                                    on master_outlet.id = transaksi_rawat_jalan.outletID join master_tindakan on transaksi_rawat_jalan.tindakanID = master_tindakan.id WHERE not transaksi_rawat_jalan.status = 'Failed' and not transaksi_rawat_jalan.status = 'Paid' and master_tindakan.typeTindakan = 'PCR' and not transaksi_rawat_jalan.barcodeLab = 'Belum dibarcode' and master_outlet.id = '$filter_outlet' and transaksi_rawat_jalan.tglTindakan like '%$date_now%' order by transaksi_rawat_jalan.transaksiID desc";
                                    }
                                }
         ...
         join master_pasien c on a.idPasien = c.id where a.idTransaction ='$idTransaction' and a.idPasien = '$pasienID'");
                                    ...
                                    <tr>
                                        ...
                                        <td><?= customDateFormat($result['sampleTime']) ?></td>
                                        <td>
                                            <?php if ($num_rows > 0) { ?>
                                        ...
                                        <td>
                                            <?php if ($num_rows <= 0) {
                                                ...
                                                if($result['status_tindakan'] =='On Process') {
                                                    if(in_array("Supervisorlab", $arrayRoleName) || in_array("Superadmin", $arrayRoleName)) {
